...
function storageKey() {
  return `atlas-ch02-${runtime.lab.session_id}`;
}
...
function updateProgressUI() {
  ...
  byId("cleared-copy").textContent = `${count}/${total} FLAGS`;
  byId("hints-used").textContent = "00";
  byId("completion-hints").textContent = "00";
  byId("completion-objectives").textContent = isComplete() ? "02 / 02" : "SOLUTION";
  ...
  byId("current-phase").textContent = isComplete()
    ...
    : isSolved("user")
      ? "02 / ROOT FLAG"
      : "01 / USER FLAG";
  byId("intel-user").textContent = isSolved("user") ? "ACCEPTED" : "PENDING";
  byId("intel-root").textContent = isSolved("root") ? "ACCEPTED" : "PENDING";
}
...
function renderFlags() {
  ...
  runtime.lab.questions.forEach((flag, index) => {
    ...
    header.append(
      element("span", "question-index", solved ? "✓" : String(index + 1).padStart(2, "0")),
    );
