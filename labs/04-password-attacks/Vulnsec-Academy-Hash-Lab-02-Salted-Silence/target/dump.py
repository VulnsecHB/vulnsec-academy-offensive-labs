#!/usr/bin/env python3
"""Northline night-lock — salted sha512crypt dump (no live cracker)."""
...
class DumpHandler(BaseHTTPRequestHandler):
    server_version = "NorthlineNightLock/2.0"
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if path == "/static/dump.css":
            ...
            return
        if path == "/":
            self.send_html(page("home.html", "Night lock — Northline", "home"))
            return
        if path == "/export":
            self.send_html(page("export.html", "Export — Northline", "export"))
            return
        if path == "/export/shadow.n1":
            self.send_bytes((APP_ROOT / "files" / "shadow.n1").read_bytes(), "text/plain; charset=utf-8")
            return
        if path == "/lock":
            self.send_html(page("lock.html", "Lock desk — Northline", "lock", notice="", desk=""))
            return
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        if (user, password) == OPERATOR:
            desk = (
                ...
                f"<h2>Night lock desk</h2><p>Welcome, {user}.</p>"
                f'<p class="flag">{FLAG}</p></section>'
            )
            notice = ""
        else:
            desk = ""
            notice = '<p class="bad">Lock rejected those credentials. Case and trailing digit matter.</p>'
        self.send_html(page("lock.html", "Lock desk — Northline", "lock", notice=notice, desk=desk))


def main() -> None:
    port = int(os.environ.get("PORT", "80"))
    server = ThreadingHTTPServer(("0.0.0.0", port), DumpHandler)
    print(f"Night lock listening on port {port}", flush=True)
