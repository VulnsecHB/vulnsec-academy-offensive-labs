...
if command -v ss >/dev/null 2>&1 && ss -ltnH | awk '{print $4}' | grep -Eq '(^|:)8888$'; then
  echo "Error: TCP port 8888 is already in use." >&2
  exit 1
fi

echo "Starting SQLi Lab 01 — Broken Gate..."

lab_subnet="10.23.54.0/24"
lab_gateway="10.23.54.1"
target_ip="10.23.54.47"

printf 'LAB_SUBNET=%s\nLAB_GATEWAY=%s\nTARGET_IP=%s\n' \
  "$lab_subnet" "$lab_gateway" "$target_ip" > "$env_file"
...
printf '{"session_id":"%s","scope":"%s","gateway":"%s","target_ip":"%s","port":80}\n' \
  "$session_id" "$lab_subnet" "$lab_gateway" "$target_ip" > "$state_file"
...
for _ in {1..40}; do
  ...
  if [[ "$portal_health" == "healthy" ]] && curl --noproxy '*' --fail --silent --max-time 2 http://127.0.0.1:8888/healthz >/dev/null 2>&1; then
    ...
    echo "Authorized subnet: $lab_subnet"
    echo "Known gateway: $lab_gateway"
    echo "Expected service: TCP 80"
    echo "Discover the staff portal yourself — do not open .lab.env."
    exit 0
  fi
  sleep 1
done
