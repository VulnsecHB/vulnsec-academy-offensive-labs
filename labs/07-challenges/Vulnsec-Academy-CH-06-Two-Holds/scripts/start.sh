...
echo "Starting Two Holds challenge..."

target_ip="10.23.54.172"
printf 'TARGET_IP=%s\n' "$target_ip" > "$env_file"

if ! docker compose up -d --build foothold samba inner decoy >"$compose_log" 2>&1; then
...
for svc in foothold samba inner decoy; do
...
for _ in {1..40}; do
  ...
  if [[ "$portal_health" == "healthy" ]] && curl --noproxy '*' --fail --silent --max-time 2 http://127.0.0.1:8888/healthz >/dev/null 2>&1; then
    echo "Two Holds is ready."
    echo "Mission Control: http://127.0.0.1:8888"
    echo "Foothold SSH: $target_ip:22"
    echo "Hidden net 10.24.20.0/24 is not reachable from Kali."
    exit 0
  fi
  sleep 1
done
