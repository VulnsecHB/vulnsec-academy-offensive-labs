...
echo "Starting Pivot Lab 03 — Second Hold..."

target_ip="10.23.54.254"
printf 'TARGET_IP=%s\n' "$target_ip" > "$env_file"

if ! docker compose up -d --build foothold hop2 vault >"$compose_log" 2>&1; then
...
for svc in foothold hop2 vault; do
...
if ipt -S DOCKER-USER >/dev/null 2>&1; then
  ...
  ipt -C DOCKER-USER -d "$DEEP_NET" -j DROP >/dev/null 2>&1 || ipt -I DOCKER-USER -d "$DEEP_NET" -j DROP
else
...
for _ in {1..40}; do
  ...
  if [[ "$portal_health" == "healthy" ]] && curl --noproxy '*' --fail --silent --max-time 2 http://127.0.0.1:8888/healthz >/dev/null 2>&1; then
    ...
    echo "Hop 2: 10.24.10.30  Vault: 10.24.20.5 (not reachable from Kali)"
    exit 0
  fi
  sleep 1
done
