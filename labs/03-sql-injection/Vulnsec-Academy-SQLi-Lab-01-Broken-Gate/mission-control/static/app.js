...
function storageKey() {
  return `atlas-sqli-lab01-${runtime.lab.session_id}`;
}
...
async function initialize() {
  ...
  try {
    ...
    loadProgress();
    document.querySelectorAll("[data-scope]").forEach((node) => { node.textContent = runtime.lab.scope; });
    document.querySelectorAll("[data-gateway]").forEach((node) => { node.textContent = runtime.lab.gateway; });
