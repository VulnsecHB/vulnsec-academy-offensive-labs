#!/usr/bin/env python3
"""Atlas Mission Control for Pivot Lab 03 — Second Hold."""
...
TARGET_IP = "10.23.54.254"
FLAG = "PIV{second_hold}"

QUESTIONS = [
    {"id":"target","eyebrow":"01 / Confirm foothold","prompt":"What is the IPv4 address of the first SSH hop?","helper":"Briefing card.","placeholder":"Target IPv4","success":"Hop 1 locked. The flag is not here.","error":"Recheck Network Coordinates.","hints":["Open Briefing.","10.23.54.254"]},
    {"id":"hop2","eyebrow":"02 / Second hop","prompt":"What IPv4 is the second SSH hop?","helper":"~/well.txt on the first foothold.","placeholder":"10.24.10.x","success":"k.well lives there. ProxyJump through hop 1.","error":"SSH as r.hold and read well.txt.","hints":["cat /home/r.hold/well.txt","10.24.10.30"]},
    {"id":"jump","eyebrow":"03 / Jump","prompt":"Which ssh flag specifies a jump host?","helper":"OpenSSH ProxyJump.","placeholder":"-?","success":"ssh -J r.hold@10.23.54.254 k.well@10.24.10.30","error":"-L is a port forward. You need a hop.","hints":["man ssh | grep jump","-J is ProxyJump.","The flag is -J."]},
    {"id":"vault","eyebrow":"04 / Third net","prompt":"What IPv4 hosts the vault HTTP on the deepest net?","helper":"~/vault.txt on hop 2.","placeholder":"10.24.20.x","success":"Forward through hop 2, or SOCKS from hop 2.","error":"SSH to k.well via -J, then cat vault.txt.","hints":["ssh -J r.hold@10.23.54.254 k.well@10.24.10.30","10.24.20.5"]},
    {"id":"flag","eyebrow":"05 / Vault","prompt":"Submit the flag from http://10.24.20.5/","helper":"Case-sensitive.","placeholder":"PIV{...}","success":"Flag accepted. Second Hold complete.","error":"ssh -J r.hold@10.23.54.254 -L 8000:10.24.20.5:80 k.well@10.24.10.30","hints":["Keep the jump + local forward up.","curl http://127.0.0.1:8000/","PIV{second_hold}"]}
]


def preview_state() -> dict[str, Any]:
    return {"session_id": "preview-second-hold", "target_ip": TARGET_IP, "port": 22}
...
def answer_is_correct(question_id: str, answer: str, state: dict[str, Any]) -> bool:
    ...
    if question_id == "hop2":
        return value.replace(" ", "") == "10.24.10.30"
    if question_id == "jump":
        compact = value.replace(" ", "")
        return compact in {"-j", "j", "proxyjump", "-jproxyjump"}
    if question_id == "vault":
        return value.replace(" ", "") == "10.24.20.5"
    if question_id == "flag":
